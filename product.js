window.onload = function () {
    ...
    var products = JSON.parse(localStorage.getItem("amazonproduct"));
  ...
  function addToCart(pro){
      alert("worked");
      var amazonproduct =JSON.stringify(pro);
      ...
      if(currentUser){
          ...
          for( var i=0; i<dataFromLs.length; i++){
              if(dataFromLs[i].email === currentUser["current-user-email"]){
                  ...
                  newObj["cartProducts"].push(JSON.parse(amazonproduct));
          ...
          // console.log()
          localStorage.setItem("amazonuserData",JSON.stringify(allUsers));
